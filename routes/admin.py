...
async def update_to_admin(user_id: int, req: Request, admin_id: int, db: Session = Depends(database.get_db)):

    updated_user = UPDATE_ADMIN(user_id, req, admin_id, db)
    return updated_user


@router.get("/get-allorders/{admin_id}",  status_code=status.HTTP_200_OK)
async def all_orders(admin_id: int, db: Session = Depends(database.get_db)):
...
async def update_order_status(order_id: int, admin_id: int, status: str, db: Session = Depends(database.get_db)):

    status = UPDATE_ORDER_STATUS(order_id, admin_id, status, db)
    return status


@router.get("/getorders-bystatus", response_model=List[schemas.OrderStatus], status_code=status.HTTP_200_OK)
async def get_orders_by_status(status: str, db: Session = Depends(database.get_db)):
